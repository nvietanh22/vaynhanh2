...
let btnSubmitMobileFirst = document.getElementById("btn-submit-first");
// let BUTTON_TEXT40 = document.getElementById("BUTTON_TEXT40");
let BUTTON_TEXT25 = document.getElementById("BUTTON_TEXT25");
let btnSubmitFirst = document.getElementById("BUTTON_TEXT25-first");
...
let btnSubmitPopupFirst = document.getElementById("btn-submit-popup-1");
...
btnSubmitFirst.addEventListener("click", () => {
  let formFirstId = '#FORM_TEXT25-first';
  let formSecondId = '#FORM_TEXT25';
  let formFirstData =  $(formFirstId).getValue();
  let rsValidate = this.validateFistForm(formFirstData);
  if (!rsValidate.valid) {
    showNotiDefault('error', 'Thất bại', rsValidate.msg);
    return;
  }
 
  $('input[name="nameWeb2"]').val(formFirstData.name);
  $('input[name="phoneWeb2"]').val(formFirstData.phone);
  $('input[name="salaryTypeWeb2"]').val(formFirstData.salaryType);
  let formSecondData =  $(formSecondId).getValue();
  console.log('formFirstData', formFirstData);
  console.log('formSecondData', formSecondData);
  $('#GROUP180-first').hide();
  $('#GROUP180').removeClass('d-none');
  $('#GROUP180').show();
});

btnSubmitMobileFirst.addEventListener("click", () => {
  let formFirstId = '#form-submit-first';
  let formFirstData =  $(formFirstId).getValue();
  let rsValidate = this.validateFistForm(formFirstData);
  if (!rsValidate.valid) {
    showNotiDefault('error', 'Thất bại', rsValidate.msg);
  ...
  $('input[name="nameMobile2"]').val(formFirstData.name);
  $('input[name="phoneMobile2"]').val(formFirstData.phone);
  $('input[name="salaryTypeMobile2"]').val(formFirstData.salaryType);
  $('#GROUP196-first').hide();
  $('#GROUP196').removeClass('d-none');
  $('#GROUP196').show();
  $("#SECTION13").css("height", "543.306px");
});
...
btnSubmitPopupFirst.addEventListener("click", (event) => {
  let formFirstId = '#form-submit-popup-1';
  let formFirstData =  $(formFirstId).getValue();
  
  $('#form-submit-popup  input[name="phone"]').val(formFirstData.phone);
  $('#form-submit-popup  input[name="name"]').val(formFirstData.name);
  $('#form-submit-popup  input[name="salaryType"]').val(formFirstData.salaryType);
  ...
  $('#modal-form-1').hide();
  $('#modal-form-2').removeClass('d-none');
});

$('#modalForm').on('hidden.bs.modal', function (e) {
  $('#modal-form-1').show();
  $('#modal-form-2').addClass('d-none');
});
  ...
  grecaptcha.ready(function () {
    grecaptcha
      ...
      .then(function (token) {
        ...
        if (prevForm == '#FORM_TEXT25') {
          phone = formData.phoneWeb2;
        } else if ('#form-submit'){
          phone = formData.phoneMobile2;
        } else {
                ...
                grecaptcha
                  ...
                  .then(function (token) {
                    ...
                    let dataOtp = {
                      TransId: uuidv4(),
                      Data: {
                        phone: dataValidate.contact_number,
                          idCard: dataValidate.national_id
                      }
                    }
      ...
      grecaptcha
          ...
          .then(function (token) {
            ...
            let salaryType = '';
              ...
              if (prevForm == '#FORM_TEXT25') {
                phone = formData.phoneWeb2;
                name = formData.nameWeb2;
                salaryType = formData.salaryTypeWeb2;
              } else if (prevForm =='#form-submit'){
                phone = formData.phoneMobile2;
                name = formData.nameMobile2;
                salaryType = formData.salaryTypeMobile2;
              } else {
                  phone = formData.phone;
                  name = formData.name;
            ...
            lib.post({
              ...
              error: function (error) {
                // Ẩn loading và xử lý khi request thất bại
                $('#loading').hide();
  ...
  grecaptcha.ready(function () {
    grecaptcha
      ...
      .then(function (token) {
        ...
        let formData = $(formId).getValue();
        let phoneOtp = '';
        if (formId == '#FORM_TEXT25') {
          phoneOtp = formData.phoneWeb2;
        } else if ('#form-submit'){
          phoneOtp = formData.phoneMobile2;
        }
...
function validateFistForm(formData) {
  console.log('formData', formData);
  let rs = {
    valid: true,
    msg: "",
